...
class ImageVideoUtils {
  ...
  static Future<void> handleImageButtonTap(
      ...
      WebImagePickImpl? webImagePickImpl,bool isPdf = false}) async {
    ...
    if (kIsWeb) {
      ...
      imageUrl = await _pickImage(imageSource, onImagePickCallback,isPdf);
    } else {
  ...
  static Future<String?> _pickImage(
      ImageSource source, OnImagePickCallback onImagePickCallback,bool isPdf) async {
    print(isPdf);
    String? path = '';
    if(isPdf){
    ...
    return onImagePickCallback(File(path!));
  }
